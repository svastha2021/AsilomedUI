...
export class SupplierProductComponent implements OnInit {
  ...
  constructor(
    private supplierProdSer: SupplierProductService,
  ...
  addRecord() {
    ...
    temp.purchase_price =
      this.supplierProductForm.controls.purchase_price.value;
  ...
  getProductList(id: any) {
    ...
    let branch_id = localStorage.getItem('branch_id');
    this.supplierProdSer
      .getSupplierProdList(bu_id, branch_id, this.eodDate)
      .subscribe(
        (data) => {
  ...
  getSupplierId() {
    ...
    let branch_id = localStorage.getItem('branch_id');
    this.supplierProdSer
      .getSupplierIdList(org_id, branch_id)
      .subscribe((data) => {
    ...
    if ($event !== '') {
      this.productList.forEach((element: any) => {
        if (element.product_name == $event) {
          this.supplierProductForm.controls.product_id.setValue(
            element.product_id
          );
        }
      });
  ...
  onChange(id: any) {
    ...
    let supplierId = id;
    this.supplierProdSer
      .getSupplierProd(branchId, supplierId)
      .subscribe((data) => {
  ...
  submit() {
    ...
    let params = {
      ...
      supplier_id: this.supplierProductForm.controls.supplier_id.value,
      supp_prods: this.tableData,
    };
  ...
  fetchProductsDynamic() {
    this.supplierProductForm.value.bu_id
      ? this.getProductList(this.supplierProductForm.value.bu_id)
      : '';
  }
  ...
  edit(item: any) {
    ...
    this.supplierProductForm.controls.purchase_price.setValue(
      item.purchase_price
    );
  ...
  editList() {
    let value = this.tableData.find(
      (element: any) => element.id == this.updateData.id
    );
    this.tableData.forEach((element: any) => {
      if (element.id == value?.id) {
        ...
        element.product_name =
          this.supplierProductForm.controls.product_name.value;
        ...
        element.credit_days =
          this.supplierProductForm.controls.credit_days.value;
      }
    });
    this.resetData();
    // this.clearFields();
